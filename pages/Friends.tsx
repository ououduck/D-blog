  ...
  return (
    <div className="py-12 md:py-20">
      ...
      <motion.div initial="hidden" animate="visible" variants={containerVariants} className="mb-16">
        <h2 className="text-2xl font-serif font-bold text-ink dark:text-white mb-6">本站信息</h2>
        <motion.div variants={itemVariants} className="p-6 md:p-8 bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col md:flex-row items-center gap-6 shadow-sm">
          <div className="w-20 h-20 rounded-full overflow-hidden bg-zinc-100 dark:bg-zinc-800 flex-shrink-0 border border-zinc-100 dark:border-zinc-700">
             <img src={siteInfo.avatar} alt={siteInfo.name} className="w-full h-full object-cover" />
          </div>
          <div className="flex-1 text-center md:text-left">
             <h3 className="font-serif font-bold text-xl text-ink dark:text-white mb-2">{siteInfo.name}</h3>
             <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">{siteInfo.description}</p>
             <code className="px-3 py-1.5 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-accent break-all select-all">
                {siteInfo.url}
             </code>
          </div>
        </motion.div>
