...
5. Issue 提交后，`friend-link-bot.yml` 会立即评论确认；提交满 25 分钟后，定时任务会检查一次友链页，通常在提交后约 25 至 30 分钟完成。
...
反链检测只请求你填写的友链页，要求页面公开可访问，并在返回的静态 HTML 中包含 `https://blog.pldduck.com/`（兼容末尾斜杠）。仅由浏览器运行脚本后动态插入的链接可能无法检测到。GitHub 的评论通知是否通过站内通知或邮件送达，取决于申请者自己的 GitHub 通知设置；D-blog 不会额外发送邮件。GitHub 定时任务受平台调度影响，不提供严格的 30 分钟 SLA。
