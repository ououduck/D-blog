...
- **构建数据校验** - `npm run gen:data` 会验证文章元数据、站内资源和友链数据，非法内容会直接导致命令失败
- **一键校验** - `npm run check` 依次执行类型检查与数据生成；数据生成会写入自动生成目录，数据非法（如日期格式错误、分类不在白名单、友链 URL 无效）会直接构建报错
...
在友链页面展开“申请友链”，按页面中的步骤完成 GitHub Issue 申请：

1. 先在自己的公开友链页加入 D-blog，并确保该页面无需登录即可访问；建议使用页面源代码中直接存在的 D-blog 链接。
2. 点击“登录 GitHub”，在 GitHub 官方页面完成登录后返回本站。本站不会读取或保存 GitHub 登录信息。
3. 填写站点名称、简介、头像地址、站点地址、友链页链接、联系方式和纯英文文件名，并勾选已添加 D-blog。
4. 点击“生成 GitHub Issue 草稿”，在 GitHub 页面检查标题和正文，最后点击 `Submit new issue` 正式提交。只生成草稿不会触发审核。
5. Issue 提交后，`friend-link-bot.yml` 会立即评论确认；提交满 10 分钟后，定时任务会检查一次友链页，通常在提交后约 10 至 15 分钟完成。
6. 检查通过后，Action 会自动写入 `friends/<filename>.json`、提交到 `main` 并关闭 Issue；访问失败、反链缺失或资料不符合规则时，会评论具体原因并关闭 Issue。失败评论会提供发送到 `i@PLDDUCK.com` 的人工审核入口，邮件会自动预填原 Issue 内容；也可以按原因修正后重新生成并提交新的 Issue。

反链检测只请求你填写的友链页，要求页面公开可访问，并在返回的静态 HTML 中包含 `https://blog.pldduck.com/`（兼容末尾斜杠）。仅由浏览器运行脚本后动态插入的链接可能无法检测到。GitHub 的评论通知是否通过站内通知或邮件送达，取决于申请者自己的 GitHub 通知设置；D-blog 不会额外发送邮件。GitHub 定时任务受平台调度影响，10 至 15 分钟是通常窗口，不是严格 SLA。

站点地址、头像地址和友链页链接必须是合法且可公开访问的 HTTP(S) 地址。文件名只允许英文字母、数字、短横线和下划线，不能包含路径；Action 和构建流程都会再次校验输入。
