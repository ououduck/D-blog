...
在友链页面展开“申请友链”，在线填写模板并输入一个纯英文文件名（例如 `my-blog`）。点击完成后，浏览器会生成并下载 `my-blog.json`，同时自动复制完整 JSON 并显示结果弹窗。
...
四个字段均为必填，`url` 与 `avatar` 必须是合法的 HTTP(S) 链接。文件名只允许英文字母、数字、短横线和下划线，不能包含路径；缺失或重复的友链文件会在构建时被跳过并告警。

申请结果弹窗中的“前往发送”会打开 `mailto:i@pldduck.com`，邮件主题必须为 `D-blog友链申请`，正文为完整 JSON。由于浏览器的 `mailto:` 协议无法可靠地自动添加附件，请在邮件客户端中手动添加刚刚下载的 `.json` 文件。详细流程请查看 [友链页面](https://blog.pldduck.com/friends)。
