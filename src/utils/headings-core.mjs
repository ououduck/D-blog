/**
 * 标题提取核心（构建与客户端共享，无依赖）：解析 Markdown 标题生成
 * TOC/锚点 id，遮蔽代码块与 HTML 注释，并处理行内 Markdown 与 HTML 实体。
 */
const INLINE_MARKDOWN_PATTERNS = [
  // URL 段支持一层嵌套括号（维基式 URL 如 .../Foo_(bar)），与 markdownText.ts
  // 的链接/图片剥离保持同一口径：此前 [^)]+ 在首个 ) 截断，标题文本被吞尾
  // （![封面](.../a_(b).png) → 「封面.png)」）。
  [/!\[([^\]]*)\]\((?:[^()]|\([^)]*\))*\)/g, '$1'],
  [/\[([^\]]+)\]\((?:[^()]|\([^)]*\))*\)/g, '$1'],
  [/`([^`]*)`/g, '$1'],
...
// Setext 分支排除列表项/引用行：CommonMark 中「- item\n- 」是两个列表项，
// 不是标题；此前 `[^\r\n]+` 会把首行当标题文本，产生幽灵 TOC 条目。
