...
function createTemplateFill(
  ...
  const stops = Array.from(
    match[2].matchAll(/(#[a-f\d]{3,8}|rgba?\([^)]*\)|hsla?\([^)]*\)|[a-z]+)\s*(\d+(?:\.\d+)?)?%?/gi),
  );
  ...
  stops.forEach((stop, index) =>
    gradient.addColorStop(
      stop[2] === undefined ? index / (stops.length - 1) : Math.min(1, Math.max(0, Number(stop[2]) / 100)),
      stop[1],
    ),
  );
  return gradient;
...
function drawCorners(ctx: CanvasRenderingContext2D, options: CoverRenderOptions, scale: number): void {
  ...
  for (const corner of [
    ...
    ctx.beginPath();
    ctx.moveTo(corner.x, corner.y + size * corner.dy);
    ctx.lineTo(corner.x, corner.y);
    ctx.lineTo(corner.x + size * corner.dx, corner.y);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(
      corner.x + size * 0.25 * corner.dx,
      corner.y + size * 0.25 * corner.dy,
      Math.max(1, 4 * scale),
      0,
      Math.PI * 2,
    );
...
function drawSeparator(ctx: CanvasRenderingContext2D, options: CoverRenderOptions, y: number, scale: number): void {
  ...
  ctx.save();
  ctx.strokeStyle = options.decorations.separatorColor;
  ctx.globalAlpha = options.decorations.separatorOpacity / 100;
  ctx.lineWidth = Math.max(1, scale);
  ctx.setLineDash([8 * scale, 6 * scale]);
  ctx.beginPath();
  ctx.moveTo((options.size.width - width) / 2, y);
  ctx.lineTo((options.size.width + width) / 2, y);
  ctx.stroke();
  ctx.restore();
}

function textX(align: TextAlign, width: number, margin: number): number {
  return align === 'left' ? margin : align === 'right' ? width - margin : width / 2;
}
...
function applyTextEffects(ctx: CanvasRenderingContext2D, options: CoverRenderOptions, scale: number): void {
  ...
  const alpha = Math.round(Math.min(1, Math.max(0, shadow.opacity)) * 255)
    .toString(16)
    .padStart(2, '0');
  const shadowColor = /^#[\da-f]{6}$/i.test(shadow.color) ? `${shadow.color}${alpha}` : shadow.color;
  ctx.shadowColor = shadow.opacity > 0 ? shadowColor : 'transparent';
...
function drawFittedText(
  ...
  maxLines: number,
  scale: number,
  fitted = fitTextBlock(ctx, text, maxWidth, fontSize, weight, options, maxLines),
): FittedText {
  const sampled = options.autoTextColor
    ? sampleRegion(
        ctx,
        align === 'left' ? x : align === 'right' ? x - maxWidth : x - maxWidth / 2,
        centerY - (fitted.lineHeight * fitted.lines.length) / 2,
        maxWidth,
        fitted.lineHeight * fitted.lines.length,
      )
    : null;
  ...
  if (decision?.lowContrast && options.diagnostics) {
    options.diagnostics.lowContrast = true;
    options.diagnostics.warnings.push('部分文字区域对比度不足，请调整背景或使用手动文字颜色');
  }
  if (fitted.truncated && options.diagnostics) {
    options.diagnostics.truncated = true;
    options.diagnostics.warnings.push('文字过长，已自动截断');
  }
  ctx.save();
  ctx.font = `${normalizeFontWeight(weight)} ${fitted.fontSize}px ${options.fontFamily}`;
  ctx.textAlign = align;
  ctx.textBaseline = 'middle';
  applyTextEffects(ctx, options, scale);
  ...
  fitted.lines.forEach((line, index) => {
    ...
    if (options.textStroke.enabled && options.textStroke.width > 0) {
      ctx.strokeStyle = options.textStroke.color;
      ctx.lineWidth = options.textStroke.width * scale;
      ctx.lineJoin = 'round';
      ctx.miterLimit = 2;
...
async function resolveIcon(source: string | CanvasImageSource): Promise<CanvasImageSource> {
  return typeof source === 'string' ? loadCachedImage(source) : source;
}
...
export async function renderCover(ctx: CanvasRenderingContext2D, options: CoverRenderOptions): Promise<void> {
  ...
  const metrics = calculateLayoutMetrics({
    ...
    fontSize: options.fontSize * baseScale,
    subFontSize: options.subFontSize * baseScale,
    iconSize: options.icon.size * baseScale,
    spacing: options.spacing * baseScale,
    subSpacing: options.subSpacing * baseScale,
    showIcon: options.icon.show,
    hasIcon: options.icon.source !== null,
    maxTextLines: options.maxTextLines,
    minFontSize: (options.minFontSize ?? 14) * baseScale,
  });
  const scale = baseScale * metrics.scale;
  const margin = CANVAS_SAFE_MARGIN * baseScale;
  ...
  diagnostics.scaled = diagnostics.scaled || metrics.scaled;
  diagnostics.overflow = diagnostics.overflow || metrics.overflow;
  ctx.save();
  ...
  if (options.backgroundImage) {
    ...
    ctx.save();
    ctx.filter = background.blur * baseScale > 0 ? `blur(${background.blur * baseScale}px)` : 'none';
    ctx.globalAlpha = background.opacity / 100;
    ctx.translate(width / 2 + background.x * baseScale, height / 2 + background.y * baseScale);
  ...
  const subSpacing = metrics.subSpacing;
  const renderOptions = {
    ...options,
    size: options.size,
    fontSize: mainSize,
    ...
    minFontSize: (options.minFontSize ?? 14) * baseScale,
    diagnostics,
    icon: { ...options.icon, size: iconSize },
  };
  const mainText =
  ...
  if (layout === 'icon-only' && options.icon.source) {
    await drawIcon(
      ...
      centerY - iconSize / 2,
      iconSize,
      options.icon.borderRadius,
      options.icon.backgroundEnabled,
      diagnostics.warnings,
    );
  } else if (layout === 'icon-split' && options.icon.source) {
    const sideWidth = Math.max(1, centerX - iconSize / 2 - spacing - margin);
    const leftFitted = options.leftText
      ? fitTextBlock(ctx, options.leftText, sideWidth, mainSize, options.fontWeight, renderOptions, mainMaxLines)
      : null;
    const rightFitted = options.rightText
      ? fitTextBlock(ctx, options.rightText, sideWidth, mainSize, options.fontWeight, renderOptions, mainMaxLines)
      : null;
    const mainHeight = Math.max(
      leftFitted?.lineHeight ? leftFitted.lineHeight * leftFitted.lines.length : 0,
      rightFitted?.lineHeight ? rightFitted.lineHeight * rightFitted.lines.length : 0,
    );
    const subtitleFitted = options.subText
      ? fitTextBlock(
          ctx,
          options.subText,
          textWidth,
          subSize,
          getSubtitleFontWeight(options.fontWeight),
          renderOptions,
          2,
        )
      : null;
    const groupHeight =
      Math.max(iconSize, mainHeight) +
      (subtitleFitted ? subSpacing + subtitleFitted.lineHeight * subtitleFitted.lines.length : 0);
    ...
    if (subtitleFitted)
      drawFittedText(
        ...
        centerX,
        mainCenterY +
          Math.max(iconSize, mainHeight) / 2 +
          subSpacing +
          (subtitleFitted.lineHeight * subtitleFitted.lines.length) / 2,
        textWidth,
        subSize,
        getSubtitleFontWeight(options.fontWeight),
        'center',
    ...
    const mainFitted = mainText
      ? fitTextBlock(ctx, mainText, textWidth, mainSize, options.fontWeight, renderOptions, mainMaxLines)
      : null;
    const subtitleFitted = options.subText
      ? fitTextBlock(
          ctx,
    ...
    const groupHeight =
      iconSize + (mainFitted ? subSpacing + mainHeight : 0) + (subtitleFitted ? subSpacing + subtitleHeight : 0);
    ...
    const x = textX(options.textAlign, width, margin);
    const align = canvasAlign(options.textAlign);
    ...
    if (subtitleFitted)
      drawFittedText(
        ...
        options.subText,
        x,
        mainCenterY + mainHeight / 2 + subSpacing + subtitleHeight / 2,
        textWidth,
        subSize,
        getSubtitleFontWeight(options.fontWeight),
        align,
    ...
    if (subtitleFitted)
      drawFittedText(
        ...
        options.subText,
        x,
        groupTop + mainHeight + subSpacing + subtitleHeight / 2,
        textWidth,
        subSize,
        getSubtitleFontWeight(options.fontWeight),
        align,
