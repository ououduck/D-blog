...
/**
 * 广告位数据（写死在此处，不再使用独立配置文件）：
 * 赞助页展示的广告横幅，增删改直接编辑本数组即可。
 */
interface AdItem {
  /** 唯一标识符 */
  id: string;
...
export const Sponsor: React.FC = () => {
  return (
    <div className="mx-auto max-w-5xl pb-12 pt-6 md:pb-20 md:pt-10">
      <Seo
        title="赞助"
        description="支持 D-blog 的多种方式：贡献代码、投稿原创文章或通过赞助链接，帮助博客持续输出高质量内容，感谢每一位支持者。"
      />
        ...
        {sponsorOptions.map((option) => {
          ...
          if (option.disabled || !option.buttonLink) {
            return (
              <article
                key={option.id}
                className="flex min-h-64 flex-col rounded-surface border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900"
              >
                {cardContent}
              </article>
            );
