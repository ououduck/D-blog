  ...
  return (
    <div className="py-12 md:py-20">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
        ...
        <p className="mt-4 inline-flex items-center gap-2 rounded-full border border-zinc-200 dark:border-zinc-800 px-4 py-2 text-sm text-zinc-500 dark:text-zinc-400">
          <Shuffle size={14} />
          以下排名不分先后，每次刷新都会随机排序
        </p>
      </motion.div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {!loading && friends.map((friend, index) => (
          <motion.a key={`${friend.url}-${index}`} variants={itemVariants} href={friend.url} target="_blank" rel="noopener noreferrer" className="group block p-6 bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 hover:border-accent/30 dark:hover:border-accent/30 hover:shadow-xl hover:shadow-accent/5 transition-all duration-300 relative overflow-hidden">
        ...
        {loading && Array.from({ length: 3 }).map((_, index) => (
          <motion.div key={`skeleton-${index}`} variants={itemVariants} className="p-6 bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 animate-pulse">
            <div className="flex items-start gap-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col max-h-[90vh] z-10">
              ...
              <div className="p-5 overflow-y-auto space-y-6">
                <div className="bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900/50 rounded-xl p-4 text-sm text-orange-800 dark:text-orange-300 leading-relaxed">
                  <strong>公告：</strong>{siteConfig.friendsPage.announcement}
                </div>

                <div>
                  <h4 className="font-bold text-ink dark:text-white mb-3 text-sm">本站信息（提交前请先添加本站友链）</h4>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-bold text-ink dark:text-white text-sm">友链 JSON 模板</h4>
                ...
                <div>
                  <h4 className="font-bold text-ink dark:text-white mb-3 text-sm">提交方式</h4>
                  <a
                    href={siteConfig.friendsPage.repoFriendsUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-between gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50 p-4 hover:border-accent/40 hover:bg-accent/5 transition-colors"
                  >
                    <div>
                    ...
                    <GitPullRequest size={18} className="text-accent flex-shrink-0" />
                  </a>
                </div>
              </div>
