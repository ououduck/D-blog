...
excerpt: 使用 GitHub Issue 收集友链申请，通过 GitHub Actions 延迟检查反链、校验公开 URL，并在审核通过后自动写入友链数据。
date: 2026-08-06
...
- `issues.opened`：只处理刚提交的友链 Issue，并发布初始确认评论。
- `schedule`：扫描已经等待足够时间的开放申请。
- `workflow_dispatch`：管理员需要立即重试或排查时手动运行审核。
...
- name: Process friend-link applications
  env:
    GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
    GITHUB_REPOSITORY: ${{ github.repository }}
    ISSUE_PAYLOAD: ${{ toJson(github.event.issue) }}
  run: node scripts/friend-link-bot.mjs ${{ github.event_name == 'issues' && 'opened' || 'review' }}
```
...
- 禁止 URL 中包含用户名或密码。
- 拒绝 `localhost` 及其子域名。
- 使用 DNS 解析所有地址，拒绝回环、私有、链路本地和保留地址。
...
export const containsBacklink = (html) => {
  ...
  const canonical = normalizeUrl('https://blog.pldduck.com/');
...
4. 确认 `.github/workflows/friend-link-bot.yml` 已经位于默认分支。
...
- 保留 `contents: write` 和 `issues: write` 这两个必要权限，不要无理由扩大权限范围。
